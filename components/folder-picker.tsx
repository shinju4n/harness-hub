  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" data-folder-picker>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl border border-gray-200 w-[500px] max-w-[90vw] max-h-[70vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Current path */}
        <div className="px-4 py-2 bg-gray-50/50 border-b border-gray-100 shrink-0">
          <p className="text-xs font-mono text-gray-500 truncate">{data.current}</p>
          {data.hasClaude && (
            <button
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {data.current !== data.parent && (
            <button
              ...
              className="w-full flex items-center gap-2 px-4 py-2.5 text-[13px] text-gray-600 hover:bg-gray-50 transition-colors border-b border-gray-50"
            >
          ...
          {loading ? (
            ...
            <div className="py-8 text-center text-gray-400 text-sm">No subfolders</div>
          ) : (
            data.folders.map((folder) => (
              <button
                key={folder.path}
                onClick={() => browse(folder.path)}
                className="w-full flex items-center gap-2 px-4 py-2.5 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors"
              >
        ...
        <div className="px-4 py-3 border-t border-gray-100 shrink-0">
          <button
            ...
            className="w-full py-2 text-xs font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors"
          >
            Select this folder
          </button>
        </div>
